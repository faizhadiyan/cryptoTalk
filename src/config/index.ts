...
function substituteEnvVariables(obj: any): any {
...
export async function loadCharacters(charactersArg: string): Promise<Character[]> {
  let characterPaths = charactersArg?.split(',').map((filePath) => {
    ...
    // If it's just a name without path or extension
    if (path.basename(filePath) === filePath) {
      // Convert to lowercase and add .character.json if not present
  ...
  if (characterPaths?.length > 0) {
    for (const characterPath of characterPaths) {
      try {
        // Try to load the character file
        let character;
        try {
          ...
          for (const altPath of alternativePaths) {
            if (fs.existsSync(altPath)) {
              character = JSON.parse(fs.readFileSync(altPath, 'utf8'));
              break;
            }
          }
        ...
        // Substitute environment variables in the character config
        const processedCharacter = substituteEnvVariables(character);

        // Handle Telegram token specifically
